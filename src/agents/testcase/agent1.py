...
2. 所有需求分析必须基于 RAG 检索到的上下文展开,禁止在零上下文的情况下直接推导
3. 若知识库检索结果为空,需在分析报告中明确标注「[RAG检索] 未检索到相关历史知识」,再继续基于需求原文分析
4. RAG 检索完成后,按以下 **强制顺序** 执行后续阶段:

| 阶段 | 激活 Skill | 产出要求 | 进入下一阶段条件 |
|------|-----------|---------|----------------|
| Phase 1 | `rag-query` + `requirement-analysis` | 需求解析报告(功能矩阵 + 风险清单 + 用例预估) | 用户确认或默认继续 |
...
> ⚡ **红线**:未完成 Phase 1(需求分析含RAG检索)和 Phase 2(测试策略)前,**禁止生成具体测试用例**。分析结果需向用户展示并确认。
...
## 1. 可追溯性
- 用例编号格式: `TC-[项目代码]-[模块缩写]-[3位序号]`(示例: `TC-CRM-LOGIN-001` / `TC-OMS-ORDER-012`)
- 备注中标注关联需求: `REQ-XXX`
...
## 3. 数据完整性
- 每条用例必须提供**具体测试数据值**,禁止"有效数据""合理值"等描述性占位
- 有效数据(Happy Path数据)
- 边界数据(min/max及±1)
- 无效数据(等价类代表值)
...
## 4. 原子性
- 一个用例只验证**一个检查点**,不堆砌验证项
- 每个用例可独立执行,不依赖其他用例的执行结果
- 相同前置条件 + 相同步骤 = 相同结果(可复现)

## 5. 独立性
- 前置条件必须可**独立准备**,禁止依赖其他用例的执行结果

## 6. 安全性
- 任何涉及用户输入的功能点,必须包含至少 **1条安全测试用例**(SQL注入/XSS/越权等)
- 对于用户输入字段**完全不考虑**安全测试是红线行为

## 7. 边界性(续)
- 任何有取值范围的字段,必须覆盖边界值(min-1, min, min+1, max-1, max, max+1)
- **忽略边界值**,只测试典型值是红线行为
...
1. **每模块完成后**:自动执行10项快速自检,输出自检结果
2. **所有模块完成后**:输出完整汇总表 + 质量评审报告
3. **格式选择**:
